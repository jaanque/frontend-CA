    ...
    if ($vn_start < $qr_res->numHits()) {
        ...
        while($qr_res->nextHit()) {
            ...
            if(($o_config->get("cache_timeout") > 0) && ExternalCache::contains($vs_cache_key,'browse_result')){
                ...
                // Obtenim només el text net per tenir control total sobre l'enllaç
                $vs_label_text = $qr_res->get("{$vs_table}.preferred_labels.name");
                
                // CORRECCIÓ APLICADA AQUÍ: 'objects' en minúscules i plural
                $vs_detail_url = caNavUrl($this->request, '', 'Detail', 'objects', array('id' => $vn_id));
                ...
                <div class='doc-item' id='row{$vn_id}'>
                    <div class='doc-content'>
                        <a href='{$vs_detail_url}' class='doc-title'>{$vs_label_text}</a>
                        <div class='doc-description'>{$vs_expanded_info}</div>
                    </div>
                    
                    <div class='doc-actions'>
                        <a href='{$vs_detail_url}' class='btn-open'>Obrir document</a>
                        {$vs_add_to_set_link}
                    </div>
                </div>";
...
<style>
    ...
    .doc-title {
        ...
        font-size: 18px;
        font-weight: 600;
        color: #111827; /* Gris gairebé negre per lectura fàcil */
    ...
    .doc-title:hover {
        color: #b21117; /* Vermell de l'AHAT al fer hover */
    }

    .doc-description {
        font-size: 14px;
        color: #6b7280; /* Gris secundari */
        line-height: 1.5;
    }
